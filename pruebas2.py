...
response = requests.get('https://www.portalinmobiliario.com/venta/departamento/propiedades-usadas/_Desde_151', headers=headers)
page=response.text.split(" ")
links=[]
for p in page:
    if "item-url=" in p:
        links.append((p.replace('"','').replace("item-url=","")))
        print((p.replace('"','').replace("item-url=","")))
print("total de links: "+str(len(links)))